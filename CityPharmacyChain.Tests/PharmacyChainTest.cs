...
/// <summary>
/// Класс, реализующий Unit-тесты
/// </summary>
/// <param name="fixture">Объект Fixture с необходимыми для тестов данными</param>
public class PharmacyChainTest(PharmacyChainFixture fixture) : IClassFixture<PharmacyChainFixture>
{
    /// <summary>
    /// Объект Fixture с необходимыми для тестов данными
    /// </summary>
    private readonly PharmacyChainFixture _fixture = fixture;

    /// <summary>
    /// Тест на корректность вывода сведений о всех препаратах в заданной аптеке, упорядоченных по названию
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Тест на корректность вывода для данного препарата подробного списка всех аптек с указанием количества препарата в аптеках
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Тест на корректность вывода информации о средней стоимости препаратов в каждой фармацевтической группе для каждой аптеки
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Тест на корректность вывода топ 5 аптек по количеству и объёму продаж данного препарата за указанный период времени
    /// </summary>
    [Fact]
        ...
        var maxProductSoldVolumes =
            (from maxProductSoldVolume in tmpMaxProductSoldVolumes
             ...
             select new
             {
                 ...
                 SoldVolume = maxProductSoldVolume.SoldVolume ?? 0,
             }).Take(5).ToList();
    ...
    /// <summary>
    /// Тест на корректность вывода списка аптек указанного района, продавших заданный препарат более указанного объёма
    /// </summary>
    [Fact]
    public void TestSelectPharmaciesWithLargeProductSoldVolume()
    {
        var tmpPharmaciesWithBigProductSoldVolumes =
            (from pharmacy in _fixture.PharmacyList
             join priceListEntry in _fixture.PriceList on pharmacy.PharmacyId equals priceListEntry.PharmacyId
             join product in _fixture.ProductList on priceListEntry.ProductId equals product.ProductId
             where pharmacy.Address is not null && pharmacy.Address.Contains("Lenin ave.") && product.Name == "Levomekol"
    ...
    /// <summary>
    /// Тест на корректность вывода списка аптек, в которых указанный препарат продается с минимальной ценой
    /// </summary>
    [Fact]
